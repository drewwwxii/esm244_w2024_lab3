...
ggplot(data = ca_counties_sf) +
  ...
  theme_void() + ##gets rid of lat/long, axes information 
  scale_fill_gradientn(colors = c("cyan","blue","purple")) #order is the order of the gradient
```
...
solano_sesb_sf <- sesb_counties_sf %>% 
  filter(county %in% county_max) ### what if two counties had the same max value?
